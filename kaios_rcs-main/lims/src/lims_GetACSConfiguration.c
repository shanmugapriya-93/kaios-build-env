...
/**
* @file lims_GetACSConfiguration.c
* @brief Implementation of lims getting ACSConfiguration functionality.
*/
...
)
{
	...
	uPalError = pal_QcmGetConfig(m->pal, ConfigurationType_Auto);
	if (uPalError != KPALErrorNone)
	{
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\tpal_QcmGetConfig() failed with error code: uPalError=%d",
