...
/**
* @file lims_AddParticipantsToGroupChatSession.c
...
)
{
	...
	uCPMError = EcrioCPMAddParticipantsToGroupChatSession(m->pCpmHandle, (EcrioCPMParticipantListStruct*)pList, pSessionId, ppReferId);
	if (uCPMError != ECRIO_CPM_NO_ERROR)
	{
		if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
			uLimsError = LIMS_SOCKET_ERROR;
		else
			uLimsError = LIMS_CPM_ADD_PARTICIPANTS_ERROR;

		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMAddParticipantsToGroupChatSession() failed with error code uCPMError:%d",
