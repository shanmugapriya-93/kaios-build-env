...
/**
* @file lims_EndSession.c
* @brief Implementation of lims end CPM session functionality.
*/
...
)
{
	...
	if (m->sessionState == lims_Session_State_IDLE)
	{
		...
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t no ongoing session",
			__FUNCTION__, __LINE__);
		goto ERR_None;
	}

	if (uSessionId == 0xFFFFFFFF)
	{
	...
	{
		uCPMError = EcrioCPMEndSession(m->pCpmHandle, pSessionId, termByUser);
		if (uCPMError != ECRIO_CPM_NO_ERROR)
		{
			if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
				uLimsError = LIMS_SOCKET_ERROR;
			else
				uLimsError = LIMS_CPM_ENDSESSION_ERROR;

			LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMEndSession() failed with error code uCPMError:%d",
...
)
{
	...
	if (m->moduleState == lims_Module_State_REGISTERED)
	{
		uCPMError = EcrioCPMAbortFileTransfer(m->pCpmHandle, pSessionId);
		if (uCPMError != ECRIO_CPM_NO_ERROR)
		{
			if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
				uLimsError = LIMS_SOCKET_ERROR;
			else
				uLimsError = LIMS_CPM_ABORT_FILETRANSFER_ERROR;

			LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t lims_AbortFileTransfer() failed with error code uCPMError:%d",
				__FUNCTION__, __LINE__, uCPMError);
		}
