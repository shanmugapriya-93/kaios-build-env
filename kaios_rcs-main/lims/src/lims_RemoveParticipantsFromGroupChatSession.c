...
/**
* @file lims_RemoveParticipantsFromGroupChatSession.c
...
)
{
	...
	if (pList == NULL)
	{
		...
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t pList is NULL",
			__FUNCTION__, __LINE__);
		goto ERR_None;
	}
	...
	if (ppReferId == NULL)
	{
		uLimsError = LIMS_INVALID_PARAMETER4;
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\tppReferId is NULL",
			__FUNCTION__, __LINE__);
		goto ERR_None;
	}

	uCPMError = EcrioCPMRemoveParticipantsFromGroupChatSession(m->pCpmHandle, (EcrioCPMParticipantListStruct*)pList, pSessionId, ppReferId);
	if (uCPMError != ECRIO_CPM_NO_ERROR)
	{
		if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
			uLimsError = LIMS_SOCKET_ERROR;
		else
			uLimsError = LIMS_CPM_REMOVE_PARTICIPANTS_ERROR;

		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMRemoveParticipantsFromGroupChatSession() failed with error code uCPMError:%d",
