...
)
{
	...
	if (EcrioCPM_SessionStatus_ACCEPT == pRespond->eStatus)
	{
		if (pSession->state != EcrioCPMSessionState_Starting)
		{
			...
			CPMLOGE(pContext->hLogHandle, KLogTypeGeneral, "%s:%u\tMaximum sessions reached uError=%u",
		...
		if (pRespond->eContentType == EcrioCPMContentTypeEnum_FileTransferOverMSRP &&
			pRespond->pFile != NULL)
		{
		...
		/*Create MSRP session*/
		uError = ec_CPM_CreateMSRPSession(pContext, &pRespond->conn, pSession, &pPath, pRespond->pFile ? Enum_TRUE : Enum_FALSE);
		...
		if (pRespond->pList != NULL &&
			pRespond->pList->uNumOfUsers != 0 && pRespond->pList->ppUri != NULL)
		{
			...
			uError = ec_CPM_FormResourceListXML(pContext, pRespond->pList, EcrioCPMParticipantType_None, (char**)&pListBuf, &uListLen);
			if (uError != ECRIO_CPM_NO_ERROR)
			{
	...
	uError = ec_CPM_PopulateFeatureTags(pContext, pRespond->eContentType, pSession->bIsLargeMessage, Enum_TRUE, Enum_FALSE, Enum_FALSE, Enum_FALSE, &inviteResp.pFetaureTags);
	if (uError != ECRIO_CPM_NO_ERROR)
	{
