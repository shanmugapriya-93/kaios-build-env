...
/**
* @file lims_SetNetworkID.c
* @brief Implementation of lims set NetworkID functionality.
*/
...
)
{
	...
	uPalError = pal_SocketSetNetID (m->pal, uNetID);
	if (uPalError != KPALErrorNone)
	{
		uLimsError = LIMS_INVALID_OPERATION;
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\tpal_SocketSetNetID() failed with error code: uPalError=%d",
