...
/**
* @file lims_RespondSession
* @brief Implementation of lims module respond session functionality.
*/
...
)
{
	...
	if (pRespond == NULL)
	{
		return LIMS_INVALID_PARAMETER3;
	}
	...
	if (m->moduleState == lims_Module_State_REGISTERED)
	{
		...
		cpmrespond.eContentType = pRespond->contentTypes;
		cpmrespond.eStatus = pRespond->eStatus;
		cpmrespond.pConvId = pRespond->pConvId;
		cpmrespond.pSessionId = pRespond->pSessionId;

		if (pRespond->pFile != NULL)
		{
			cpmrespond.pFile = pRespond->pFile;
		}
		else
		{
			cpmrespond.pFile = NULL;
		}

		uCPMError = EcrioCPMRespondSession(m->pCpmHandle, &cpmrespond);
		if (uCPMError != ECRIO_CPM_NO_ERROR)
		{
			if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
				uLimsError = LIMS_SOCKET_ERROR;
			else
				uLimsError = LIMS_CPM_RESPONDSESSION_ERROR;

			LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMRespondSession() failed with error code uCPMError:%d",
