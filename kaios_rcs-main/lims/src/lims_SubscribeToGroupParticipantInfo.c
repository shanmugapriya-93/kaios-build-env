...
/**
* @file lims_SubscribeToGroupParticipantInfo.c
...
)
{
	...
	uCPMError = EcrioCPMSubscribeToGroupParticipantInfo(m->pCpmHandle, pSessionId, pGroupSessionId);
	if (uCPMError != ECRIO_CPM_NO_ERROR)
	{
		if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
			uLimsError = LIMS_SOCKET_ERROR;
		else
			uLimsError = LIMS_CPM_SUBSCRIBE_PARTICIPANT_INFO_ERROR;

		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMSubscribeToGroupParticipantInfo() failed with error code uCPMError:%d",
...
#endif
#else
	pGroupSessionId = pGroupSessionId;
...
)
{
	...
	uCPMError = EcrioCPMUnSubscribeToGroupParticipantInfo(m->pCpmHandle, pSessionId, pGroupSessionId);
	if (uCPMError != ECRIO_CPM_NO_ERROR)
	{
		if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
			uLimsError = LIMS_SOCKET_ERROR;
		...
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMUnSubscribeToGroupParticipantInfo() failed with error code uCPMError:%d",
			__FUNCTION__, __LINE__, uCPMError);
		goto ERR_None;
	}
