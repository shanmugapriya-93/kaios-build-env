...
/**
* @file lims_RemoveCapabilities.c
* @brief Implementation of lims Remove Capabilities.
*/
...
/*	uUCEError = EcrioUCEPublishCapabilityInfo(m->pUCEHandle, (u_char*)pETag,NULL, EcrioUCE_Publish_Remove);

	if (uUCEError != ECRIO_UCE_NO_ERROR)
	{
		uLimsError = LIMS_UCE_PUBLISH_REMOVE_ERROR;

		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioUCERemoveParticipantsFromGroupChatSession() failed with error code uUCEError:%d",
