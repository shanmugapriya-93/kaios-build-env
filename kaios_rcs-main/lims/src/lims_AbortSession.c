...
/**
* @file lims_AbortSession.c
* @brief Implementation of lims Abort CPM session functionality.
*/
...
)
{
	...
	if ((handle == NULL) || (ppSessionId == NULL) || (*ppSessionId == NULL) || (uCount <= 0))
	{
	...
	{
		uCPMError = EcrioCPMAbortSession(m->pCpmHandle, ppSessionId[uIndex]);
		if (uCPMError != ECRIO_CPM_NO_ERROR)
		{
			uLimsError = LIMS_CPM_ABORT_SESSIONID_ERROR;

			LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMAbortSession() failed with error code uCPMError:%d",
