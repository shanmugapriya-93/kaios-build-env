...
* @param[in] pIcon		The string of icon.
* @param[in] pFileInfo	The file-info structure.
...
)
{
	...
	EcrioCPMGroupIconFileInfoStruct fileInfo = { 0 };
#endif
	...
	dmStruct.icon.action = EcrioCPMDataActionType_Set;

	if (pIcon != NULL)
	{
		dmStruct.icon.pIconUri = pIcon;
	}
	else if (pFileInfo != NULL)
	{
		pal_MemorySet((void*)&fileInfo, 0, sizeof(EcrioCPMGroupIconFileInfoStruct));
		
		fileInfo.pContentType = pFileInfo->pContentType;
		fileInfo.pData = pFileInfo->pData;
		fileInfo.pFileInfo = pFileInfo->pFileInfo;
		fileInfo.uDataLength = pFileInfo->uDataLength;

		dmStruct.icon.pFileInfo = &fileInfo;
	}
	else
	{
		uLimsError = LIMS_INVALID_PARAMETER2;
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t pIcon and pFileInfo is NULL",
