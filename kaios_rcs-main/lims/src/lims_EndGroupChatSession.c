...
/**
* @file lims_EndGroupChatSession.c
* @brief Implementation of lims end CPM Group Chat session functionality.
*/
...
)
{
	...
	{
		uCPMError = EcrioCPMEndGroupChatSession(m->pCpmHandle, pSessionId, leaveGroup);
		if (uCPMError != ECRIO_CPM_NO_ERROR)
		{
			if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
				uLimsError = LIMS_SOCKET_ERROR;
			else
				uLimsError = LIMS_CPM_END_GROUPCHATSESSION_ERROR;

			LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t EcrioCPMEndGroupChatSession() failed with error code uCPMError:%d",
