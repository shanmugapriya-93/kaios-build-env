...
)
{
	...
	if (pHostUri == NULL)
	{
		...
		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t pHost is NULL",
			__FUNCTION__, __LINE__);
		goto ERR_None;
	}

	dmStruct.host.action = EcrioCPMDataActionType_Move;
	dmStruct.host.pUserRole = (char*) "Administrator";
	dmStruct.host.pHostUri = pHostUri;
