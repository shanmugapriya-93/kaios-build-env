...
/**
* @file lims_DeleteGroupChatSubject.c
...
)
{
	...
	dmStruct.subject.action = EcrioCPMDataActionType_Delete;
	dmStruct.subject.pSubjectLine = NULL;
	...
	if (uCPMError != ECRIO_CPM_NO_ERROR)
	{
		if (uCPMError == ECRIO_CPM_SOCKET_ERROR)
			...
			uLimsError = LIMS_CPM_DELETE_GROUPCHAT_SUBJECT_ERROR;
