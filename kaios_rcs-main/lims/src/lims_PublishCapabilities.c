...
/**
* @file lims_PublishCapabilities.c
* @brief Implementation of lims Publish Capabilities.
*/
...
* @param[in] pUserCapabilities	The list of capabilities that need to be published
...
)
{
	...
	if (pETag == NULL)
	{
		uUCEError = EcrioUCEPublishCapabilityInfo(m->pUCEHandle, (u_char*)pETag, pUserCapabilities, EcrioUCE_Publish_Initiate);
	}
	else
	{
		uUCEError = EcrioUCEPublishCapabilityInfo(m->pUCEHandle, (u_char*)pETag, pUserCapabilities, EcrioUCE_Publish_Modify);
	}
	...
	if (uUCEError != ECRIO_UCE_NO_ERROR)
	{
		...
		if (pETag == NULL)
			uLimsError = LIMS_UCE_PUBLISH_INITIATE_ERROR;
		else
			uLimsError = LIMS_UCE_PUBLISH_MODIFY_ERROR;

		LIMSLOGE(m->logHandle, KLogTypeGeneral, "%s:%u\t lims_PublishCapabilities() failed with error code uUCEError:%d",
...
ERR_None:
	LIMSLOGI(m->logHandle, KLogTypeFuncExit, "%s:%u\t%u", __FUNCTION__, __LINE__, uLimsError);
